...
public class DrawioXmlProcessor {
    ...
    /**
     * Extracts all IDs from an XML string using regex.
     */
    public static Set<String> extractAllIds(String xml) {
    ...
    /**
     * Sanitize an XML fragment by remapping duplicate or root IDs (0 and 1) to UUIDs,
     * and syncing references (parent, source, target) to match the new IDs.
     * Also drops mistakenly generated default roots (id=0 and id=1 with parent=0).
     */
    public static String sanitizeAndRemapIds(String fragmentXml, Set<String> existingIds) {
        if (fragmentXml == null || fragmentXml.trim().isEmpty()) {
            return "";
        }
        try {
            DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
            DocumentBuilder builder = factory.newDocumentBuilder();
            // Wrap in <wrapper> to ensure it's valid XML even if it has multiple root elements
            String wrappedXml = "<wrapper>" + fragmentXml + "</wrapper>";
            Document doc = builder.parse(new ByteArrayInputStream(wrappedXml.getBytes(StandardCharsets.UTF_8)));
            ...
            // Pass 1: Identify nodes to remove and nodes to remap
            for (int i = 0; i < allNodes.getLength(); i++) {
                Element el = (Element) allNodes.item(i);
                if (el.getTagName().equals("wrapper")) {
                    continue;
                }
                
                if (el.hasAttribute("id")) {
                    String originalId = el.getAttribute("id");
                    String parent = el.getAttribute("parent");

                    // Check if AI mistakenly generated the root nodes
                    if ("0".equals(originalId) && (parent == null || parent.isEmpty())) {
                        elementsToRemove.add(el);
                        continue;
                    }
                    if ("1".equals(originalId) && "0".equals(parent)) {
                        elementsToRemove.add(el);
                        continue;
                    }

                    // Remap if ID is "0" or "1" (not removed above), or if it exists in the diagram,
                    // or if we've already seen this ID in the current fragment (duplicate generated by AI)
                    if ("0".equals(originalId) || "1".equals(originalId) || existingIds.contains(originalId)) {
                        // Generate a short 8-character hex string for the new ID to keep XML concise
                        String newId = "cell-" + UUID.randomUUID().toString().substring(0, 8);
            ...
            // Remove unwanted root nodes
            for (Element el : elementsToRemove) {
                el.getParentNode().removeChild(el);
            }

            // Refresh the NodeList after removal
            allNodes = doc.getElementsByTagName("*");

            // Pass 2: Update references (parent, source, target)
            for (int i = 0; i < allNodes.getLength(); i++) {
                Element el = (Element) allNodes.item(i);
                if (el.getTagName().equals("wrapper")) {
                    continue;
                }
                updateAttributeIfMapped(el, "parent", idMapping);
                updateAttributeIfMapped(el, "source", idMapping);
                updateAttributeIfMapped(el, "target", idMapping);
            }

            // Extract the inner XML of <wrapper>
            ...
            // Fallback to the old regex method if DOM parsing fails
            return extractMxCellsOnly(fragmentXml);
        }
    }

    private static void updateAttributeIfMapped(Element el, String attrName, Map<String, String> idMapping) {
        if (el.hasAttribute(attrName)) {
            String val = el.getAttribute(attrName);
            if (idMapping.containsKey(val)) {
                el.setAttribute(attrName, idMapping.get(val));
            }
        }
    }
